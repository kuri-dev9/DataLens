...
시나리오는 **현재 확보된 PoC Dataset으로 실제 완주 가능해야 한다.** `DB_PoC.md` / `DB_PoC_Relationship.md`
확인 결과 현재 데이터는 `MME / 0016` 단일 노드 140행(2024-05-29 11:20 ~ 2024-05-30 11:29)이다.
따라서 **엔티티 비교가 아니라 시간축 드릴다운과 관계 확장**을 축으로 하는 다음 시나리오를 대표 시나리오로 한다.

```
U1: "MME_16의 시간대별 KPI를 보여줘."
    → active_period 주입, NODE_TYPE=MME AND NODE_ID=0016
    → CM_EPC_INFO로 장비명 해석 (복합 키 관계, ADR-027)
    → ds_000001 (시간대별 집계)

U2: "성공률이 가장 낮았던 시간대는 언제야?"
    → "그중" 없이 직전 결과 참조. DB 재조회 없이 transform
    → ds_000002 (sort + limit)

U3: "그때 주요 실패 원인은?"
    → "그때" = ds_000002의 1행 시간 구간으로 해소
    → PM_EPC_CAUSE_1M으로 문맥 전파 (JOIN 아님, ADR-027 R-6)
    → ds_000003 (Cause별 집계)

U4: "이 MME에 연결된 링크를 보여줘."
    → active_entities = MME/0016 유지
    → PM_LINK_EPC_KPI_1M을 NODE1/NODE2 양방향 탐색 (ADR-027 R-4)
    → 상대 endpoint가 VLAN/Interface일 수 있음
    → ds_000004 (Link 목록)

U5: "그중 문제가 큰 링크의 원인은?"
    → "그중" = ds_000004 → 상위 Link 선택
    → PM_LINK_EPC_ROOT_CAUSE_1M으로 문맥 전파
    → ds_000005
```

이 5턴이 안정적으로 동작하는 것이 PoC의 핵심 인수 조건이다.

이 시나리오는 다음을 모두 통과한다: 복합 키 관계(U1), DB 미접근 transform(U2), 지시 표현 해소(U3·U5),
대칭 endpoint 탐색(U4), 문맥 전파형 관계(U3·U5).

#### 현재 Dataset 제약과 그 영향

| 제약 | 영향 | 대응 |
|---|---|---|
| 노드가 `MME/0016` 1대 | "A랑 C가 이상한데" 형태의 **엔티티 비교·선택 시나리오를 실데이터로 검증할 수 없다** | 해당 경로는 fixture 기반 테스트로 검증. 다중 장비 데이터 확보 시 대표 시나리오에 복원 (ADR 액션 아이템 #10) |
| 데이터가 2024-05-29~30 | `active_period` 기본값("어제")을 주입하면 **0행이 반환된다** | PoC 환경의 기본 기간을 `partitions.yaml`에서 데이터 존재 구간으로 설정. 기본값 주입 로직 자체는 동일하게 검증됨 |
| 140행 | 성능·부하·프루닝 효과를 실측할 수 없다 | NFR-010은 `EXPLAIN` 기반 프루닝 적용 여부로 판정 (행 수 무관) |
| `CL_SGW` / SGW Fact 없음 | 유형별 Master 확장 경로 미검증 | Schema 지원은 유지. `DEFERRED` 관계로 선언 (ADR-027 R-3) |

**현재 Dataset은 기능 완주 검증에는 사용할 수 있으나 운영 대표성이나 Metric 품질 판단의 근거로 사용하지 않는다.**
...
5. **해석 공개 의무 (ADR-020).** Domain Pack에 선언되지 않은 용어를 해석했거나,
   `confirmed`가 아닌 상태의 관계를 사용했다면 응답에 반드시 명시한다.

   ```
   "'소통률'을 성공률(SUCCESS_CNT / ATTEMPT_CNT)로 해석했습니다."
   "이 관계는 스키마 구조로 추정한 것이며 실데이터로 확인되지 않았습니다."
   ```

   이는 친절함이 아니라 **구조적 요구사항**이다. 의미 오류는 유효한 SQL과 그럴듯한 숫자를 만들어
   fail-closed 안전망에 걸리지 않으므로, 사용자에게 부정 신호를 낼 기회를 주는 것이 유일한 탐지 경로다.
...
**시그니처 4개는 동결한다(ADR-010 개정).** 기능 확장은 인터페이스 추가가 아니라 **반환 타입에
`source`와 `status`를 포함**하는 방식으로 한다.

| 필드 | 값 | 의미 |
|---|---|---|
| `source` | `pack` / `session` / `fk` / `inferred` | 어디서 온 지식인가 |
| `status` | `declared` / `candidate` | Query 생성에 직접 사용 가능한가 (ADR-020) |

`candidate`는 사용 시 반드시 응답에 공개한다(8.4 규칙 5). 미검증 관계를 fail-closed 경로에 노출하는
`get_relationship_candidate()` 같은 신규 시그니처는 두지 않는다.

PoC 구현체는 `DictionarySemanticProvider`(Domain Pack YAML 기반) 하나다(ADR-010).

호출 이전에 **표기 정규화(ADR-021)** 를 적용한다. NFKC·공백 제거·비율/개수 접미사 정규화·조사 제거로
`성공률 / 성공율 / 성공 비율 / 성공퍼센트`를 하나의 정규형으로 수렴시키며,
그 결과 Domain Pack에 대규모 alias를 나열할 필요가 없다.

### 14.4 세션 학습과 Semantic 경계
...
이 원칙은 ADR-020에서 정식 결정으로 승격되었다. 요약하면 다음과 같다.

1. Semantic의 Source of Truth는 **Domain Pack**이다. 런타임 학습물은 Truth가 될 수 없다.
2. 상태는 `declared` / `candidate` 두 가지뿐이다. 다단계 lifecycle과 confidence decay는 두지 않는다.
3. **침묵은 증거가 아니다.** 실행 성공·반복 사용·무이의는 신뢰도에 기여하지 않는다.
   명시적 사용자 확인·교정만 증거로 인정한다.
4. `candidate` → `declared` 자동 승격은 없다. Domain Pack diff에 대한 사람 승인으로만 이루어진다.
5. 미해소 용어는 추측하지 않고 되묻는다(UC-010, FR-016).

#### Clarify-Confirm-Log 루프

```
미해소 용어
   │
   ▼
후보 제시 (did_you_mean 확장)  ─── "'소통률'을 찾지 못했습니다. 1) 성공률  2) 접속 성공률"
   │
   ▼
사용자 확인
   ├──▶ session_aliases 즉시 반영        (FR-005)
   └──▶ Candidate Store 기록 → Pack diff 제안 → 사람 승인 (15.2)
```

이 루프는 **추론된 매핑이 아니라 사용자가 확정한 매핑**을 얻는다. 신규 컴포넌트가 없고,
추가 LLM 호출이 없으며, P-4(결정적으로 확정 가능한 것을 LLM에 맡기지 않는다)에 정합한다.
대가는 첫 만남의 명확화 1턴이며 이는 UC-010에 이미 정의된 정상 동작이다.
...
### 15.4 Vector 도입 결정의 분할 (ADR-010 개정)

"Vector DB / RAG"를 하나의 결정으로 묶어 두면 부분 도입 논의가 불가능하다. 세 개로 분할한다.

| 결정 | 상태 | 재평가 게이트 |
|---|---|---|
| Schema Retrieval (벡터 스키마 검색) | PoC 도입 검토 대상 아님 | 대상 테이블이 수백 규모로 증가할 때 |
| Query Memory Retrieval (유사 질의 검색) | `DEFERRED` | Interaction Log 3~6개월 축적 후 |
| 문서 RAG (운영 매뉴얼·KPI 설명) | `DEFERRED` | 24장 우선순위 3 그대로 |

Schema Retrieval을 제외하는 근거는 **PoC Core Scope가 11개 테이블**이라는 점이다(`DB_PoC.md`).
11개 테이블의 메타데이터는 스키마 컨텍스트 예산 2,500 토큰에 충분히 들어가므로 후보를 좁힐 대상이 없다.

### 15.5 Interaction Log (PoC 범위, ADR-024)

Vector 검색은 미루되 **기록은 PoC에서 수행한다.** 데이터는 소급 생성이 불가능하지만
인덱스는 언제든 소급 생성이 가능하기 때문이다.

성공·실패한 상호작용을 JSONL로 남긴다. 기록 대상은 발화 원문, 정규화형, 해소 결과, QuerySpec,
실행 결과 메타이며 **행 데이터 값은 기록하지 않는다**(NFR-011).

| 용도 | 설명 |
|---|---|
| Golden set 확장 | 실사용 발화를 GQ- 항목으로 승격. 1인 체제의 최대 병목인 검증 자산을 자동 축적 |
| 표현 분포 관측 | alias 관리 부담 문제의 실재성을 데이터로 판정 (ADR-020 재검토 근거) |
| 회귀 탐지 | 동일 발화의 QuerySpec 변화 감지 |
| NFR 실측 | steps, latency가 그대로 지표 |

기록의 중심을 SQL이 아니라 **QuerySpec**에 두는 것이 핵심이다. QuerySpec은 DBMS·dialect·물리 스키마에
종속되지 않으므로 장기 재사용성이 SQL보다 높다.
...
1. 대표 시나리오(3.1) 5턴 완주 — **현재 Dataset(MME/0016)으로 실제 실행 가능한 시나리오 기준**
2. golden set 단일턴 정답률 80% 이상 (NFR-007)
3. 멀티턴 5턴 완주율 60% 이상 (NFR-008)
4. 파티션 프루닝 미적용 쿼리 0건 (NFR-010) — `EXPLAIN` 기반 판정
...
| P-11 | Semantic의 Source of Truth는 Domain Pack이다. 런타임 학습물은 Truth가 될 수 없다 (ADR-020) |
| P-12 | 추론으로 해석한 의미는 반드시 사용자에게 공개한다. 침묵은 동의가 아니다 (ADR-020) |
| P-13 | 검색·유사도 결과는 후보이지 진실이 아니다. 진실은 구조화 검증으로 확정한다 (ADR-010 개정) |
...
| N-17 | 식별자 컬럼에 SUM/AVG 등 수치 집계 | 의미 없는 값을 조용히 반환 (ADR-022) |
