...
> ## ⚠ 문서 상태: 제안 (Proposal) — 일부 미채택
>
> **본 문서는 확정 설계가 아니다.** 검토 결과 상당 부분이 기각·수정·연기되었다.
> 구현 근거로 사용해서는 안 되며, 확정 결정은 `ADR.md`와 `DATALENS-SAD.md`에만 존재한다.
>
> | 항목 | 내용 |
> |---|---|
> | 상태 | Proposal / 부분 채택 |
> | 판정 문서 | `SEM-REVIEW-001.md` (Semantic Architecture 제안 비판적 검토) |
> | 확정 결정 | `ADR.md` ADR-020 ~ ADR-027, ADR-009 개정, ADR-010 개정 |
> | 검토일 | 2026-08-10 |
>
> ### 주요 미채택 항목 (본문을 그대로 따르지 말 것)
>
> | 본문 | 판정 | 확정 결정 |
> |---|---|---|
> | §2.2 · §8 General Lexicon (WordNet 계열 ETL) | **기각** | ADR-021 — 사전 대신 결정적 표기 정규화 |
> | §9 · §24 Semantic Learner / 자동 학습 | **기각** | ADR-020 — Clarify-Confirm-Log 루프, 자동 승격 금지 |
> | §15 Semantic Catalog가 Source of Truth | **기각** | ADR-009 개정 — Domain Pack이 SoT로 유지 |
> | §6 · §24 Confidence 기반 승격 (0.72 → 0.92) | **기각** | ADR-020 — 상태는 `declared`/`candidate` 2개. 침묵은 증거가 아님 |
> | §10 Semantic Scope 4단계 | **수정** | ADR-020 — `pack` / `session` 2단계 |
> | §28 Semantic Retrieval Pipeline 9단계 | **기각** | ADR-010 개정 — 컨텍스트 예산 초과, 지연 33% 증가 |
> | §32 "PoC에서 Phase 1~3" | **기각** | ADR-010 개정 — Vector는 PoC 도입 검토 대상 아님 (Core 11개 테이블) |
> | §11 Query Memory **검색** | **연기** | ADR-010b — 게이트: 이력 3~6개월 축적 후 |
> | §11 QuerySpec을 기록 중심으로 | **채택** | ADR-024 — 기록은 PoC 범위, 검색은 연기 |
> | §5.1 Identifier Detection | **채택 (위치 변경)** | ADR-022 — QueryForge Validation의 결정적 규칙으로 |
> | §4 Data Profiling | **채택 (제약 추가)** | ADR-023 — 파티션 한정 + CLI 전용. 무제약 스캔은 N-8 위반 |
> | §7 · §22 복합 Relationship | **채택 (승격)** | ADR-027 — `relationships.yaml` 선언 스키마 요구사항 |
> | §13 · §29 Vector는 Truth가 아님 | **채택** | P-13 (SAD 28.1) |
> | §23 enrich-context 항목 목록 | **채택 (흡수)** | ADR-009 개정 — Domain Pack `semantic.yaml` 필드로 |
> | §17 · §26 WrenAI 파일 참조 | **조건부** | ADR-025 — 설계 참고만 허용, 코드 파생은 라이선스 확인 전 금지 |
>
> §33의 "구현 시 재검토 사항" 12건 중 6건은 위 결정으로 소멸했고, 2건은 축소, 2건은 연기되었다.
> 실제로 남은 결정은 Query Memory 보존 정책과 개인정보 저장 정책이며 **둘 다 ADR-024에서 확정되었다.**
